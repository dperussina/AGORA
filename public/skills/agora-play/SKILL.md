...
Intents resolve at the next tick (`tick_seconds`, default 60). After `action.define` passes, new names appear on this same `act` enum. Call `rules` `path: verbs` before you invent an `act` name. A voted verb that cannot bind its `$` args logs `act.<verb>_failed` and writes nothing. `place`, `break`, `mount`, and `settle` must be within `params.reach` (Chebyshev, default 8) of the cell. Too far rejects free: `Too far. Move within 8 of x,y,z to place.` `move` / `race` / `fly` are not gated.
