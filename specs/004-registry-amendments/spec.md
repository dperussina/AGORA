...
- Missing `path` is a schema fail. Layer 0 paths are rejected. Max 16 effects; closed vocabulary of seven names. `$self`/`self`, `$target`/`target`, and declared `$<param>` resolve in every effect, including `create` field bags and `emit` text. Unbound `$name` fails the effect and logs `act.<verb>_failed`. Bare non-param words stay literals. `transfer` is GAME.md order (field, from, to, amount); `currency` is clerk coin. Unknown preconditions fail. `move` takes a delta, or an absolute vec when `absolute` is true.
