...
- `observe` returns `lore` `{ world, volume, cell }` and narration includes voted lore plus the mark text when present.
...
- `inspect` targets: identity id, `x,y,z` / `cell:x,y,z` (lore stack), anchor designation / `ANCHOR:<id>`, `warden:<id>`, drift id, or `ent:<n>` after a creature vote (`010`). Warden / Drift / automaton replies include `personifies`, `createdBy`, and `text.types.<type>.lore` if set. Echoes are observational (`act` targeting `echo:` rejects). There is no quest tool.
